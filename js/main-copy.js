...
var OFFER_TITLES = [
  "Уютный дом с видом на сад",
  "Стильная комната в лучшем районе города",
  "Квартира рядом с метро",
  "Сдается комната на 3 месяца",
...
var getRoomsAndGuests = function (offer) {
  ...
  if (offer.offer.rooms && offer.offer.guests) {
    roomsAndGuests =
      offer.offer.rooms + " комнаты для " + offer.offer.guests + " гостей";
  }
...
var renderOneMapCard = function (offer, cardTemplate) {
  ...
  if (offer.offer.photos.length > 0) {
    ...
    photos.innerHTML = "";
    for (
      var photoIndexInOffer = 0;
      photoIndexInOffer < offer.offer.photos.length;
      photoIndexInOffer++
    ) {
...
mapCardBlock.insertBefore(
  cardElement,
  mapCardBlock.querySelector(".map__filters-container")
);
